...
function FlyToController({ target, globeRef, controlsRef }) {
  ...
  useEffect(() => {
    ...
    if (prevRoundRef.current === target.round) return;
    prevRoundRef.current = target.round;

    const pos = globeRef.current.getCoords(target.lat, target.lng, 0);
    if (!pos) return;

    const dir = new Vector3(pos.x, pos.y, pos.z).normalize();
    const dist = camera.position.length();
    targetPosRef.current = dir.multiplyScalar(dist);
  ...
  }, [target, globeRef, controlsRef, camera]);

  useFrame(() => {
    if (!animatingRef.current || !targetPosRef.current) return;

    camera.position.lerp(targetPosRef.current, 0.035);
    camera.lookAt(0, 0, 0);

    if (camera.position.distanceTo(targetPosRef.current) < 0.5) {
...
export function World({ globeConfig, data, races, onHoverChange, onFrontRaceChange, flyToTarget }) {
  ...
  return (
    ...
    >
      ...
      <directionalLight color={globeConfig.directionalLeftLight} position={[-400, 100, 400]} intensity={1.4} />
      <directionalLight color={globeConfig.directionalTopLight} position={[-200, 500, 200]} intensity={1.2} />
      <pointLight color={globeConfig.pointLight} position={[-200, 500, 200]} intensity={1.8} />
      ...
      <FlyToController target={flyToTarget} globeRef={globeRef} controlsRef={controlsRef} />
      <OrbitControls
        ref={controlsRef}
        enablePan={false}
        enableZoom={false}
